...
import 'package:iacula_app/features/meditation/presentation/meditation_detail_screen.dart';

void main() {
  testWidgets('text meditations load remote content into reading mode', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      ProviderScope(
        ...
        child: CupertinoApp(home: MeditationDetailScreen(item: item)),
      ),
    );
    ...
    expect(find.text('Modo leitura'), findsOneWidget);
    expect(find.text('Texto sincronizado'), findsOneWidget);
    expect(find.text('Primeiro parágrafo.'), findsOneWidget);
    expect(find.text('Segundo parágrafo.'), findsOneWidget);
    expect(find.text('Abrir fonte original'), findsOneWidget);
  });
}
